...
PVOID WINAPI resolve_delay_loaded_api(PVOID parent_module_base, PCIMAGE_DELAYLOAD_DESCRIPTOR delayload_descriptor, PVOID failure_dll_hook, PVOID failure_system_hook, PIMAGE_THUNK_DATA thunk_address, ULONG flags) {
    ...
    NT_DECL(ResolveDelayLoadedAPI) = (PVOID)GetProcAddress(kernelbase, "ResolveDelayLoadedAPI");

    char *dllName = (char *) ((DWORD64) parent_module_base + delayload_descriptor->DllNameRVA);
    HMODULE dllHandle = GetModuleHandleA(dllName);
    if (!dllHandle) {
        DEBUG_MEDIUM("[+] resolved_delay_loaded_api: delayed load for %s\n", dllName);
...
BOOL snapping(PE *dll_parsed) {
    ...
    for (SIZE_T i = 0; importDescriptor->Name; importDescriptor++) {
        ...
        // We are going roots here... The GetModuleHandle set the error code
        // if the DLL is not found. If we find it with LoadLibrary, just reset
        // the error code.
        // Otherwise it is propagated to the entrypoint result...
        if(dll_handle){
            SetLastError(0);
        }
...
API_SET_VALUE_ENTRY * api_setp_search_for_api_set_host(API_SET_NAMESPACE_ENTRY *api_namespace_entry, wchar_t *api_to_resolve, unsigned __int16 api_to_resolve_size, API_SET_NAMESPACE *api_namespace) {
    ...
    NT_DECL(RtlCompareUnicodeStrings) = (PVOID)GetProcAddress(ntdll, "RtlCompareUnicodeStrings");
    ...
    // WORKSHOP TODO: What type of research algorithm is used here ?
    if (up_index >= 1) {
        do {
            // WORKSHOP TODO: Fix this, the found_entry_index is the result of a computation including up_index and bot_index
            ULONG found_entry_index = 0;
        ...
        // WORKSHOP TODO: Fix the while condition
        } while (FALSE);
    }
...
NTSTATUS api_set_resolve_host(API_SET_NAMESPACE *api_namespace, PUNICODE_STRING api_to_resolve, PUNICODE_STRING parent_name, int *resolved, PUNICODE_STRING resolved_dll) {
    ...
    *resolved = 0;
    ...
    if (api_to_resolve->Length >= 8) {
        ...
        // WORKSHOP TODO: According to the NTDLL, fix the if condition
        if (FALSE) {
            ...
            // WORKSHOP TODO: Fix the if condition according to the NTDLL code
            if (FALSE) {
                // Get the entry related to the ApiSet to resolve
                // WORKSHOP TODO: Call the right function 
                api_namespace_entry = NULL;
                if (api_namespace_entry) {
                    ...
                    // WORKSHOP TODO: Fix the if condition
                    } else if (FALSE) {
