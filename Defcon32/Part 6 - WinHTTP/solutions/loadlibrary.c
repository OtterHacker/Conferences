...
ULONG ldr_hash_entry(UNICODE_STRING UniName, BOOL xor_hash) {
    ULONG hash = 0;
    HMODULE ntdll = get_module_handle(DLL_NTDLL, NULL);
    NT_DECL(RtlHashUnicodeString) = get_proc_address(ntdll, FCT_RTLHASHUNICODESTRING);
...
PRTL_RB_TREE rewind_tree(PLDR_DATA_TABLE_ENTRY ldr_entry, PRTL_BALANCED_NODE node){
    ...
    if (!node->Red) {
        ...
        for (int i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
            if (hash_string(section->Name, 0, NULL) == SEC_DATA) {
...
PRTL_RB_TREE find_ldrp_module_mapping_info_index() {
    ...
    PLDR_DATA_TABLE_ENTRY ldr_entry = get_ldr_entry(DLL_NTDLL);
...
BOOL add_mapping_info_module(PLDR_DATA_TABLE_ENTRY ldr_entry, PE *dll) {
    HMODULE ntdll = get_module_handle(DLL_NTDLL, NULL);
    NT_DECL(RtlRbInsertNodeEx) = get_proc_address(ntdll, FCT_RTLRBINSERTNODEEX);
    ...
    PLDR_DATA_TABLE_ENTRY ldr_node = (PLDR_DATA_TABLE_ENTRY) ((size_t) ldrp_module_mapping_info_index->Root - offsetof(LDR_DATA_TABLE_ENTRY, MappingInfoIndexNode));
    bRight = 0;
    BOOL bRight1 = 0;
    while (1) {
...
PVOID link_module_to_peb(PE *dll, LPWSTR DllPath, LPWSTR DllName) {
    ...
    HMODULE ntdll = get_module_handle(DLL_NTDLL, NULL);
    NT_DECL(RtlInitUnicodeString) = get_proc_address(ntdll, FCT_RTLINITUNICODESTRING);
    NT_DECL(NtQuerySystemTime) = get_proc_address(ntdll, FCT_NTQUERYSYSTEMTIME);
...
PVOID search_for_ldrp_inverted_function_table(PVOID *mr_data, PULONG mr_data_size) {
    ...
    PVOID ntdll = get_module_handle(DLL_NTDLL, NULL);
    ...
    *mr_data = NULL;
    for (DWORD i = 0; i < pe->ntHeader->FileHeader.NumberOfSections; i++, section_header++) {
        if (hash_string((char *) section_header->Name, 0, NULL) == SEC_MRDATA) {
...
PVOID minimal_memory_map(LPWSTR filepath) {
    ...
    //PVOID address = 0x0007ff469ea0000;
    PVOID address = NULL;
...
PVOID WINAPI resolve_delay_loaded_api(PVOID parent_module_base, PCIMAGE_DELAYLOAD_DESCRIPTOR delayload_descriptor, PVOID failure_dll_hook, PVOID failure_system_hook, PIMAGE_THUNK_DATA thunk_address, ULONG flags) {
    HMODULE kernelbase = get_module_handle(DLL_KERNELBASE, NULL);
    NT_DECL(ResolveDelayLoadedAPI) = get_proc_address(kernelbase, FCT_RESOLVEDELAYLOADEDAPI);

    char *dllName = (char *) ((DWORD64) parent_module_base + delayload_descriptor->DllNameRVA);
    HMODULE dllHandle = get_module_handle(hash_string(dllName, 0, NULL), NULL);
    if (!dllHandle) {
        DEBUG_NATIVE("[+] resolved_delay_loaded_api: delayed load for %s\n", dllName);
        //dllHandle = load_library_a(dllName);
        dllHandle = load_library_a(dllName);
    }
    ...
    // we don't have to manually reprotect the IAT table and it only cost
...
BOOL snapping(PE *dll_parsed) {
    ...
    for (SIZE_T i = 0; importDescriptor->Name; importDescriptor++) {
        ...
        HMODULE dll_handle = get_module_handle(hash_string(dll_name, 0, NULL), NULL);
        if (!dll_handle) {
            DEBUG_LOW("\t\t[-] DLL not found, load the DLL\n");
            dll_handle = load_library_a(dll_name);

            if (!dll_handle) {
                if (dll_name[0] == 'e' && dll_name[1] == 'x' && dll_name[2] == 't' && dll_name[3] == '-') {
                    continue;
                }
                DEBUG_LOW("\t\t[x] snapping: cannot load library\n");
                return FALSE;
            }
        }
        ...
        for (; ilt->u1.Function; iat++, ilt++) {
            if (IMAGE_SNAP_BY_ORDINAL(ilt->u1.Ordinal)) {
                ...
                iat->u1.Function = (DWORD_PTR)get_proc_address_ordinal(dll_handle, function_ordinal);
            } else {
                ...
                if (hash_string(hint->Name, 0, NULL) == FCT_LOADLIBRARYA) {
                    iat->u1.Function = (DWORD_PTR)GET_SYMBOL(load_library_a);
                } else if (hash_string(hint->Name, 0, NULL) == FCT_LOADLIBRARYW) {
                    iat->u1.Function = (DWORD_PTR)GET_SYMBOL(load_library_w);
                } else if (hash_string(hint->Name, 0, NULL) == FCT_LOADLIBRARYEXW) {
                    iat->u1.Function = (DWORD_PTR)load_library_ex_w;
                } else if (hash_string(hint->Name, 0, NULL) == FCT_LOADLIBRARYEXA) {
                    iat->u1.Function = (DWORD_PTR)load_library_ex_a;
                } else if (hash_string(hint->Name, 0, NULL) == FCT_FREELIBRARY) {
                    iat->u1.Function = (DWORD_PTR)free_library;
                } else if (hash_string(hint->Name, 0, NULL) == FCT_RESOLVEDELAYLOADEDAPI) {
                    iat->u1.Function = (DWORD_PTR)resolve_delay_loaded_api;
                } else {
                    DEBUG_LOW("\t\t[+] snapping: function Name : %s\n", hint->Name);
                    iat->u1.Function = (DWORD_PTR) get_proc_address(dll_handle, hash_string(hint->Name, 0, NULL));
...
API_SET_NAMESPACE_ENTRY *api_setp_search_for_api_set(API_SET_NAMESPACE *api_namespace, wchar_t *api_to_resolve, SHORT api_to_resolve_size) {
    PAPI_SET_HASH_ENTRY hash_entry = NULL;
    ULONG api_to_resolve_hash = 0;
    API_SET_NAMESPACE_ENTRY *found_entry = NULL;
    ULONG offset = 0;

    // Just ensure that the APISet name is in lowercase
    // Avoid the use of external function that may be stored
    // in the crt dll instead of NTDLL...
    for (int i = 0; i < api_to_resolve_size; i++) {
        wchar_t api_to_resolve_lower = api_to_resolve[i];
        if (api_to_resolve[i] > 65 && api_to_resolve[i] < 90) {
            api_to_resolve_lower += 32;
        }
        // Compute a hash that can be used to identify the ApiSet
        api_to_resolve_hash = api_to_resolve_lower + api_namespace->HashFactor * api_to_resolve_hash;
    }

    ULONG bot_index = 0;
    ULONG up_index = api_namespace->Count - 1;
    if (up_index < 0)
        return found_entry;

    while (1) {
        // Dichotomy algorithm to search for the ApiSet hash in
        // the ApiNameSpace
        ULONG index = (up_index + bot_index) >> 1;
        offset = api_namespace->HashOffset + sizeof(DWORD64) * index;
        hash_entry = (API_SET_HASH_ENTRY *) ((DWORD64) api_namespace + offset);

        // The Api Namespace table is sorted by hash value
        // So see if the next dichotomy index must be lower
        // or greater than the current one
        if (api_to_resolve_hash < hash_entry->Hash) {
            up_index = index - 1;
        } else if (api_to_resolve_hash <= hash_entry->Hash) {
            // We found the entry, let's get out
            break;
        } else {
            bot_index = index + 1;
        }
        if (bot_index > up_index)
            return found_entry;
    }
    // Get the entry real address
    found_entry = (API_SET_NAMESPACE_ENTRY *) ((DWORD64) api_namespace +
                                               sizeof(API_SET_NAMESPACE_ENTRY) * hash_entry->Index +
                                               api_namespace->EntryOffset);

    // Using the hash_string, lets compare the UNICODE_STRING
    // without RtlCompareUnicodeString
    if (!found_entry || hash_string(api_to_resolve, api_to_resolve_size * 2, NULL) == hash_string((PWCH)((DWORD64) api_namespace + found_entry->NameOffset), found_entry->HashedLength, NULL)){
        return found_entry;
    }
    return 0;
}
...
API_SET_VALUE_ENTRY * api_setp_search_for_api_set_host(API_SET_NAMESPACE_ENTRY *api_namespace_entry, wchar_t *api_to_resolve, unsigned __int16 api_to_resolve_size, API_SET_NAMESPACE *api_namespace) {
    HMODULE ntdll = get_module_handle(DLL_NTDLL, NULL);
    NT_DECL(RtlCompareUnicodeStrings) = get_proc_address(ntdll, FCT_RTLCOMPAREUNICODESTRINGS);

    ULONG value_offset = api_namespace_entry->ValueOffset;
    ULONG bot_index = 1;
    ULONG up_index = api_namespace_entry->ValueCount - 1;
    API_SET_VALUE_ENTRY *default_entry = (API_SET_VALUE_ENTRY *) ((DWORD64) api_namespace + value_offset);
    if (up_index >= 1) {
        do {
            ULONG found_entry_index = (up_index + bot_index) >> 1;
            API_SET_VALUE_ENTRY *found_entry_temp = (API_SET_VALUE_ENTRY *) ((DWORD64) api_namespace +
                                                                             sizeof(API_SET_VALUE_ENTRY) *
                                                                             found_entry_index + value_offset);
            int compare_result = win32_RtlCompareUnicodeStrings(
                    api_to_resolve,
                    api_to_resolve_size,
                    (PWCH) ((DWORD64) api_namespace + found_entry_temp->NameOffset),
                    found_entry_temp->NameLength >> 1,
                    1
            );

            if (compare_result < 0) {
                up_index = found_entry_index - 1;
            } else {
                if (compare_result <= 0) {
                    return (API_SET_VALUE_ENTRY *) ((DWORD64) api_namespace + sizeof(API_SET_VALUE_ENTRY) * found_entry_index + value_offset);
                }
                bot_index = found_entry_index + 1;
            }
        } while (bot_index <= up_index);
    }
...
NTSTATUS api_set_resolve_host(API_SET_NAMESPACE *api_namespace, PUNICODE_STRING api_to_resolve, PUNICODE_STRING parent_name, int *resolved, PUNICODE_STRING resolved_dll) {
    wchar_t *buffer;
    DWORD64 api_to_resolve_start;
    DWORD64 result;
    unsigned int api_to_resolve_length;
    API_SET_NAMESPACE_ENTRY *api_namespace_entry;
    API_SET_VALUE_ENTRY *host_library_entry = NULL;

    *resolved = 0;
    resolved_dll->Buffer = NULL;
    resolved_dll->Length = 0;
    resolved_dll->MaximumLength = 0;
    if (api_to_resolve->Length >= 8) {
        buffer = api_to_resolve->Buffer;

        // No clue on this one
        api_to_resolve_start = *(QWORD *) buffer & 0xFFFFFFDFFFDFFFDF;

        // Check that the Api to resolve start with api- or ext-
        if (api_to_resolve_start == 0x2D004900500041 || api_to_resolve_start == 0x2D005400580045) {
            api_to_resolve_length = api_to_resolve->Length;

            // Format the ApiSet name to throw out every thing after the last -
            // So api-ms-win-core-processthreads-l1-1-0.dll became api-ms-win-core-processthreads-l1-1
            for (int i = 0; i < api_to_resolve->Length >> 1; i++) {
                api_to_resolve_length -= 2;
                if (api_to_resolve->Buffer[(api_to_resolve->Length >> 1) - i - 1] == '-') {
                    break;
                }
            }

            // The >> 1 is heavily used to get the size in number of char
            // from a string stored as a UNICODE_STRING
            if (api_to_resolve_length >> 1) {
                // Get the entry related to the ApiSet to resolve
                api_namespace_entry = api_setp_search_for_api_set(api_namespace, buffer, (SHORT) (api_to_resolve_length >> 1));
                if (api_namespace_entry) {
                    if (parent_name && api_namespace_entry->ValueCount > 1) {
                        // If the ApiSet is asked by a forwarder, this branch will
                        // ensure that the ApiSet does not resolve to the calling
                        // function.
                        // This seems to be mainly used when forwarder are involved.
                        host_library_entry = api_setp_search_for_api_set_host(
                                api_namespace_entry,
                                parent_name->Buffer,
                                parent_name->Length >> 1,
                                api_namespace);
                    } else if (api_namespace_entry->ValueCount) {
                        host_library_entry = (API_SET_VALUE_ENTRY *) ((DWORD64) api_namespace + api_namespace_entry->ValueOffset);
                    } else {
                        host_library_entry = NULL;
                    }

                    // Finally store the DLL related to the API
                    // in a UNICODE_STRING
                    if (host_library_entry) {
                        *resolved = 1;
                        resolved_dll->Buffer = (wchar_t *) ((DWORD64) api_namespace + host_library_entry->ValueOffset);
                        resolved_dll->MaximumLength = (USHORT)host_library_entry->ValueLength;
                        resolved_dll->Length = (USHORT)host_library_entry->ValueLength;
                    }
                }
            }
        }
...
int resolve_api_set(PWSTR api_set, PWSTR resolved_api, PWSTR parent) {
    int resolved = 0;
    UNICODE_STRING resolved_host_library;
    PPEB peb = (PPEB) GetPEB();

    // Get the ApiSetMap that contains the mapping between the
    // ApiSets and the DLLs
    PAPI_SET_NAMESPACE api_set_namespace = (PAPI_SET_NAMESPACE) peb->ApiSetMap;
    SHORT api_set_length = 0;
    while(api_set[api_set_length] != L'\0'){api_set_length += 1;}
    UNICODE_STRING api_to_resolve = {
            .Buffer = api_set,
            .Length = api_set_length * sizeof(wchar_t),
            .MaximumLength = api_set_length * sizeof(wchar_t)
    };
    UNICODE_STRING parent_unicode;
    if(parent){
        SHORT parent_length = 0;
        while(parent[parent_length] != L'\0'){parent_length += 1;}
        parent_unicode.Buffer = parent;
        parent_unicode.Length = parent_length * sizeof(wchar_t);
        parent_unicode.MaximumLength = parent_length * sizeof(wchar_t);
    }
    NTSTATUS Status = api_set_resolve_host(
            api_set_namespace,
            &api_to_resolve,
            parent == NULL ? NULL : &parent_unicode,
            &resolved,
            &resolved_host_library
    );
    if (resolved) {
        for (int i = 0; i < resolved_host_library.Length >> 1; i++) {
            resolved_api[i] = resolved_host_library.Buffer[i];
        }
        resolved_api[resolved_host_library.Length >> 1] = L'\0';
        return resolved_host_library.Length >> 1;
    }
    return resolved;
}

void resolve_dll_path(LPWSTR initial_dll_name, LPWSTR *dll_name, DWORD *dll_name_size, LPWSTR *dll_path,
                      DWORD *dll_path_size) {
    ...
    DWORD resolved = resolve_api_set(resolved_api, resolved_api, NULL);
    if(resolved){
        extension[0] = L'\0';
    }
...
PVOID ldr_load_dll(LPWSTR filepath, HANDLE file, DWORD dwFlags){
    ...
    PVOID already_loaded = get_module_handle(hash_string(dll_name, 2 * dll_name_size, NULL), NULL);
    if (already_loaded) {
        DEBUG_MEDIUM("[-] ldr_load_dll: module %ls is already loaded\n", dll_name);
        // Increment the load count for the DLL
        insert_module_base_address_node(already_loaded, NULL, NULL, NULL);
        free(dll_path);
        return already_loaded;
    }
    DEBUG_MEDIUM("[+] ldr_load_dll: loading %ls\n", dll_name);
    if (!dll_name) {
        free(dll_path);
        return NULL;
    }
