...
void inject_shellcode() {
    ...
    while (1) {
        ...
        if (memInfo.State == MEM_COMMIT && memInfo.Protect == XX) {
    ...
    BOOL result = WriteProcessMemory((HANDLE)XX, XX, shellcode, shellcode_size, &szOutput);
    if(result == FALSE) {
    ...
    //       still be executable
    result = VirtualProtect(XX, page_size, XX, &oldProtect);
    if(result == FALSE) {
