...
DWORD64 InstrumentationCHook(DWORD64 Function, DWORD64 ReturnValue){
    ...
    if (g_Recurse == 0){
        ...
        if(XXXXXXXXXX){
            ...
            CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)XXXXXX, XXXXXX, 0, NULL);
        }
        g_Recurse = 0;
    }
    return result;
...
VOID awake(PVOID lpParam){
    ...
    if (lpParam != NULL){
        ...
        memcpy((PVOID)XX, stackBackup, stackBackupSize);
        free(stackBackup);

        // We can change the RSP in the thread context
        // TODO: Fix this call to set the new RSP pointer
        ((PCONTEXT)lpParam)->Rsp = (DWORD64)XX;

        // TODO: Restore the thread with NtContinue
        NtContinue(XX, FALSE);
    }
    else{
        mainProgram();
...
VOID NirvanaSleep(ULONGLONG time){
    ...
    //       its stackframe...
    threadCtxBackup.Rip = *(PDWORD64)(XX);
