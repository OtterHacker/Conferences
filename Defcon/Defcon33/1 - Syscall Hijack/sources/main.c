...
DWORD64 InstrumentationCHook(DWORD64 calling_function, DWORD64 initial_sysret){
    // TODO: fix this function so that it changes the SYSRET when NtAllocateVirtualMemory is called
    // Hint: Look at the ASM hook code and check which register should contains the
    //       SYSRET once the execution ends.
    //       In x64, the calling convention says that the function return value is
    //       stored in RAX
    ...
    // TODO: play with the stop_recursive_call to understand why this is needed :)
    ...
    if (stop_recursive_call == 0){
        ...
        // TODO: Ensure that this condition is True only for NtAllocateVirtualMemory
        if (symbol_resolving_status && issubstr(symbol->Name, XXXXXXXXXXXX)) {
            ...
            // TODO: you might want to make something here...
        }
...
int main(void) {
    ...
    // TODO: Configure the right callback function for this hook
    InstrumentationCallbackInfo.Callback = XXXXX;
    ...
    // TODO: Fix the call to NtSetInformationProcess
    // Hint: To configure a NirvanaHook, the option is ProcessInstrumentationCallback
    //       We need to give the structure defining the hook to NtSetInformationProcess
    ...
    // TODO: If you remove the while(1), you will see that the hook is called
    //       multiple time. Why ?
    while(1){}
}
