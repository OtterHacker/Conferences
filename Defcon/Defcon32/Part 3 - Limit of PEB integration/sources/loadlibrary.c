...
PVOID search_for_ldrp_inverted_function_table(PVOID *mr_data, PULONG mr_data_size) {
    ...
    *mr_data = NULL;
    ...
    while ((DWORD64) candidate <= (DWORD64) *mr_data + *mr_data_size - sizeof(RTL_INVERTED_FUNCTION_TABLE)) {
        // WORKSHOP TODO: Try to understand the different checks performed in this function
        // The mindset used to fix this bug is always used when trying to reimplement the
        // NTDLL
        ...
        // already loaded)
        // WORKSHOP TODO: Using the NTDLL information, complete the condition
        if (
                ldr_table->Count > 0 &&
                FALSE
        ) {
            ...
            for (unsigned int i = 0; i < ldr_table->Count; i++) {
                ...
                if (
                    ...
                    // WORKSHOP TODO: In this case, is it a valid candidate or note ? Replace 9999999 by 0 or 1
                    valid = 9999999;
                    break;
                } else {
...
void rtlp_insert_inverted_function_table_entry(PVOID image_base, ULONG image_size, PVOID exception_directory,
    ...
    // WORKSHOP TODO: Fix this call by replacing the 2 NULL value by the right values
    if (!VirtualProtect(NULL, mr_data_size, NULL, &old_protect)) {
    ...
    if (ldrp_inverted_function_table->Count == ldrp_inverted_function_table->MaxCount) {
        ...
        if (ldrp_inverted_function_table->Count != 1) {
            ...
            // WORKSHOP TODO: Use the NTDLL implementation to fix the while condition
            while (FALSE) {
        ...
        // WORKSHOP TODO: Use the NTDLL to fix the if condition
        if (FALSE) {
            memmove(
    ...
    // WORKSHOP TODO: Replace the NULL values by the right values using the NTDLL
    ldrp_inverted_function_table->Entries[entry_index].ImageBase = NULL;
    ldrp_inverted_function_table->Entries[entry_index].ImageSize = NULL;
    ldrp_inverted_function_table->Entries[entry_index].ExceptionDirectory = NULL;
    ldrp_inverted_function_table->Entries[entry_index].ExceptionDirectorySize = NULL;
    ...
    // WORKSHOP TODO: Fix this call
    VirtualProtect(NULL, mr_data_size, NULL, &old_protect);
}
